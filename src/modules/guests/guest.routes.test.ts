...
import { makeBooking, makeGuest } from '../../../tests/factories';

function buildApp(): Express {
  return createApp(createTestDb());
}

describe('guests over the wire', () => {
  it('takes a new one and lists it back', async () => {
    ...
    const made = await api(app)
      ...
      .send({ name: 'A. Visitor', greenFeePence: 4500 });
    expect(made.status).toBe(201);

    const listed = await api(app).get(`/bookings/${bookingId}/guests`);
  ...
  it('reads one back by its id, and 404s for one that is not there', async () => {
    ...
    const made = await api(app)
      .post(`/bookings/${bookingId}/guests`)
      .send({ name: 'A. Visitor', greenFeePence: 4500 });
    const read = await api(app).get(`/guests/${made.body.id}`);
    expect(read.status).toBe(200);
    expect(read.body.id).toBe(made.body.id);

    const missing = await api(app).get('/guests/999999');
  ...
  it('turns down a body carrying a field it does not know', async () => {
    ...
    const res = await api(app)
      .post(`/bookings/${bookingId}/guests`)
      .send({ name: 'A. Visitor', greenFeePence: 4500, nonesuch: 1 });
  ...
  it('turns down a query it does not know', async () => {
    ...
    const res = await api(app).get(`/bookings/${bookingId}/guests?nonesuch=1`);
    expect(res.status).toBe(400);
  });
  ...
  it('gives back only as many as were asked for', async () => {
    const app = buildApp();
    const bookingId = await makeBooking(app);
    await makeGuest(app, { bookingId });
    await makeGuest(app, { bookingId });

    const all = await api(app).get(`/bookings/${bookingId}/guests`);
    expect(all.body.items).toHaveLength(2);

    const one = await api(app).get(`/bookings/${bookingId}/guests?limit=1`);
  ...
  it('turns down an id that is not a number', async () => {
    ...
    const res = await api(app).get('/guests/not-an-id');
    expect(res.status).toBe(400);
  });
  ...
  it('turns down a page size of nothing', async () => {
    ...
    const res = await api(app).get(`/bookings/${bookingId}/guests?limit=0`);
    expect(res.status).toBe(400);
  });

  it('will not sign a guest onto a round that has been played', async () => {
    const app = buildApp();
    const bookingId = await makeBooking(app);

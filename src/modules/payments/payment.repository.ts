...
export class PaymentRepository {
  ...
  findById(id: number): Payment | null {
    const row = this.db.prepare('SELECT * FROM payments WHERE id = ?').get(id) as unknown as
  ...
  list(page: Page, filter: PaymentFilter = {}): Payment[] {
    ...
    const rows = this.db
      .prepare(`SELECT * FROM payments ${where} ORDER BY paid_on ASC, id ASC LIMIT ? OFFSET ?`)

...
describe('payments over the wire', () => {
  it('takes a new one and lists it back', async () => {
    ...
    const made = await api(app)
      .post('/payments')
      .send({ invoiceId: invoiceId, paidOn: '2025-06-05', method: 'card', amountPence: 1000 });
  ...
  it('brings the outstanding balance down and refuses an overpayment', async () => {
    ...
    const part = await api(app)
      .post('/payments')
      .send({ invoiceId, paidOn: '2025-06-05', method: 'card', amountPence: 1000 });
    ...
    const tooMuch = await api(app)
      .post('/payments')
      .send({ invoiceId, paidOn: '2025-06-06', method: 'cash', amountPence: gross });

...
import { makeClub, makeStaffMember } from '../../../tests/factories';
...
describe('staff over the wire', () => {
  ...
  it('turns down a query it does not know', async () => {
    ...
    const res = await api(app).get(`/clubs/${clubId}/staff?nonesuch=1`);
    expect(res.status).toBe(400);
  });

  it('refuses a second one with the same payroll_number', async () => {
    const app = buildApp();
    const clubId = await makeClub(app);
  ...
  it('gives back only as many as were asked for', async () => {
    const app = buildApp();
    const clubId = await makeClub(app);
    await makeStaffMember(app, { clubId });
    await makeStaffMember(app, { clubId });

    const all = await api(app).get(`/clubs/${clubId}/staff`);
    expect(all.body.items).toHaveLength(2);

    const one = await api(app).get(`/clubs/${clubId}/staff?limit=1`);
  ...
  it('turns down a page size of nothing', async () => {
    ...
    const res = await api(app).get(`/clubs/${clubId}/staff?limit=0`);
    expect(res.status).toBe(400);
  });
  ...
  it('404s when amending one that is not there', async () => {
    ...
    const res = await api(app).patch('/staff/999999').send({ role: 'secretary' });
    expect(res.status).toBe(404);
  });
});
